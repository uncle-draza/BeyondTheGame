...
public class EnemySightAI : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (distanceToPlayer < killDistance+attackDistance && isAttacking)
        {
            animationState = 3;
        }
        ...
        if (player.GetComponent<PlayerController>().isTranslating == true && clearLOStoPlayer == true && isPlayerInFOV && distanceToPlayer < maxSightDistance)
        {
            isAttacking = true;
